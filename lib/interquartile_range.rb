...
# the medians of the values above or below the Q2. The IQR is equal to Q3 - Q1. Here's an example:
...
# Make a function that takes an array of floats and/or integers and returns the 
# IQR for that array. The return type can be float or int. It doesn't matter.
...
# Notes
# In all test cases, none of the solutions have repeating decimals, so you shouldn't 
# need to worry about rounding the result.
# There is no universal agreement on how to select quartile values, so if you use an 
# online calculator (such as Wolfram Alpha), you could get a different result depending 
# on how the quartile values were calculated. You need to obtain the quartile values in 
